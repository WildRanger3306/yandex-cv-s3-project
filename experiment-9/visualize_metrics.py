...
Выгружает loss, lr, grad_norm, identity_similarity и возвращает (или сохраняет) картинку.
...
def fetch_metric(run_id, metric_key, mlflow_url=MLFLOW_URL):
...
def get_metrics_image(run_id, mlflow_url=MLFLOW_URL):
    """
    Принимает run_id, выгружает метрики и возвращает объект PIL.Image.
    """
    data = {}
    for key in METRICS:
        steps, values = fetch_metric(run_id, key, mlflow_url)
    ...
    for i, cfg in enumerate(CONFIGS):
        ...
        if not steps:
            ax.text(0.5, 0.5, f"No data for {cfg['key']}", color="white", ha="center")
            continue
    ...
    # Сохраняем в буфер и возвращаем PIL Image
    buf = io.BytesIO()
...
if __name__ == "__main__":
    ...
    print(f"Выгрузка метрик из MLflow для {RUN_ID}...")
